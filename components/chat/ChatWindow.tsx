        ...
        {messages.map((message, index) => (
          <div key={`${message.role}-${index}`} className={message.role === 'user' ? 'text-right' : 'text-left'}>
            {(() => {
              ...
              const isPendingAssistant = isLatestAssistant && isLoading;
              const hasContent = message.content.trim().length > 0;

              if (isPendingAssistant && !hasContent) {
                return (
                  <div className="inline-block max-w-[85%] rounded-2xl rounded-tl-none border border-borderColor bg-white px-3 py-2 text-sm text-textMuted">
                    <LoadingSpinner size="sm" label="답변 생성 중" className="mb-2" />
              ...
              return (
                ...
                >
                  {message.content}
                  {isPendingAssistant && hasContent ? (
                    <div className="mt-2 flex items-center gap-1 text-[11px] font-semibold text-textMuted">
